...
parameters {
  ...
  vector<lower=0>[2] sd_P; //RN parameter sds
  cholesky_factor_corr[2] LP; //RN parameter correlations
  matrix[I,2] std_dev; //individual-level RN deviations
...
matrix[2,2] R = LP * LP'; //RN correlation matrix
matrix[2,2] S = diag_matrix(sd_P); //RN correlation matrix
matrix[2,2] P = S*R*S; //RN covariance
vector<lower=0>[2] V_P = sd_P .* sd_P; //RN variances
}
