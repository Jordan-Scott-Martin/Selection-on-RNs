...
generator_function1 = function(I, rep_z, rep_w){
  ...
  #random effects
  sd_I_z = rep(1,3) #RN parameter sds
  LP = t(chol(rethinking::rlkjcorr(1,3,10))) #RN parameter correlations
...
backend1 = SBC_backend_cmdstan_sample(SBC_mod1, iter_sampling = 500, iter_warmup = 500, 
                                      init = 0, chains = 4,  parallel_chains = 4, 
                                      adapt_delta = 0.80, max_treedepth = 10) 

#test with single dataset
res = SBC_mod1$sample(data = df1$generated[[1]], iter_sampling = 500, iter_warmup = 1000,
                      parallel_chains = 4, init = 0.001, adapt_delta = 0.9)
print(df1$variables[1,], max_variables = 18)
...
results1 =  compute_SBC(datasets = df1, backend = backend1,
                        ...
                        keep_fits = T)
saveRDS(results1, "results_SBC_NLS.RDS")
...
#plot results
#bp =  
  plot_ecdf_diff(results1, variables = paste0("b","[",1:3,"]"), prob = 0.95)
#qp =   
  plot_ecdf_diff(results1, variables = paste0("q","[",1:3,"]"), prob = 0.95)
#qcp = plot_ecdf_diff(results1, variables = paste0("qc","[",1:3,"]"), prob = 0.95)
bp = plot_sim_estimated(results1, estimate = "median", variables = paste0("b","[",1:3,"]"))
qp = plot_sim_estimated(results1, estimate = "median", variables = paste0("q","[",1:3,"]"))

bp$data$bias = (bp$data$simulated_value - bp$data$estimate__)  
qp$data$bias = (qp$data$simulated_value - qp$data$estimate__) 

bp$data$efs = ifelse(abs(bp$data$simulated_value) < 0.3, "small", 
                     ifelse(abs(bp$data$simulated_value) < 0.5, "medium",
                     ifelse(abs(bp$data$simulated_value) < 1, "large",
                     ifelse(abs(bp$data$simulated_value) > 1, "extreme", NA))))

qp$data$efs = ifelse(abs(qp$data$simulated_value) < 0.3, "small", 
                     ifelse(abs(qp$data$simulated_value) < 0.5, "medium",
                            "large"))


ggplot(qp$data, aes(y = bias, x = efs)) + geom_boxplot()
